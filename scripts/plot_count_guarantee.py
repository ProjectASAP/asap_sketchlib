#!/usr/bin/env python3
"""Plot Count Sketch within-count guarantee charts from metrics.csv.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot Count Sketch within-count guarantee charts from metrics.csv"
    )
...
def plot_group(path_kind: str, dist: str, rows: list[dict[str, float]], out_path: Path) -> None:
    ...
    ax.set_yscale("log")
    ax.set_title(f"Count Sketch guarantee ({path_kind}, {dist})")
    ax.set_ylabel("count")
