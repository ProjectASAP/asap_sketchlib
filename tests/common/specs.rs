...
pub fn cdf_sup_distance(estimated: &[(f64, f64)], sorted_truth: &[f64]) -> (f64, f64) {
    ...
    // The jump points of both functions.
    let mut points: Vec<f64> = Vec::with_capacity(n + estimated.len());
    points.extend(sorted_truth.iter().copied());
    points.extend(estimated.iter().map(|(v, _)| *v));
    ...
    for &x in &points {
        let truth = sorted_truth.partition_point(|t| t.total_cmp(&x) != std::cmp::Ordering::Greater)
            as f64
            / n as f64;
        let idx =
            estimated.partition_point(|(v, _)| v.total_cmp(&x) != std::cmp::Ordering::Greater);
        let hat = if idx == 0 { 0.0 } else { estimated[idx - 1].1 };
        let d = (hat - truth).abs();
        if d > worst {
            worst = d;
            worst_at = x;
...
// ---------------------------------------------------------------------------
// Self-tests for the specs themselves
// ---------------------------------------------------------------------------
