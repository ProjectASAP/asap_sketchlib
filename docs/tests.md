...
| `cs_metadata_rejects_unknown_keys` | An unexpected metadata key fails closed. | Encodes the ten `CsMetadata` fields plus a `bogus_field` as a named map and verifies it does not decode as `CsMetadata`. |
| `cs_metadata_rejects_counter_type_key` | Count-Min's `counter_type` key is not part of Count Sketch metadata. | Encodes the ten `CsMetadata` fields plus `counter_type: "i64"` as a named map and verifies it does not decode as `CsMetadata`. |
...
| `space_saving_every_key_type_round_trips_and_keeps_its_variant` | The `key_type` names the exact `HeapItem` variant and is never widened. | Across all 13 wire key types - `i8`, `i16`, `i32`, `i64`, `isize`, `u8`, `u16`, `u32`, `u64`, `usize`, `f32`, `f64`, and `string` reached from `Str`, `String`, and `Bytes` alike - verifies the metadata carries the expected name, the bytes re-serialize identically, and every decoded key still answers its original `DataInput` with the weight it was given; a decoder widening `i32` to `i64` keeps the digest but stops comparing equal, so `estimate` would read zero. |
...
| `space_saving_rejects_a_key_type_the_payload_does_not_carry` | A payload relabelled with another `key_type` is refused. | Re-frames a string-keyed payload under a `u64` `key_type` and a `u64`-keyed payload under `string`, and verifies neither decodes. |
