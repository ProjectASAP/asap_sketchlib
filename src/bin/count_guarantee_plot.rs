...
use sketchlib_rust::{Count, FastPath, RegularPath, SketchInput, Vector2D};
...
const DEFAULT_OUT_DIR: &str = "data/results/count_guarantee";
...
#[derive(Parser, Debug)]
#[command(name = "count_guarantee_plot")]
#[command(about = "Generate Count Sketch epsilon/delta guarantee metrics")]
struct Cli {
...
fn run(cli: &Cli) -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "Count Sketch guarantee artifacts written to {}",
        run_dir.display()
    );
    println!(
        "generate plots with: python3 scripts/plot_count_guarantee.py --metrics {}/metrics.csv --out-dir {}",
        run_dir.display(),
        run_dir.display()
    );
    Ok(())
    ...
    F: Fn(u64) -> f64,
{
    ...
    for (&key, &true_count) in truth {
        ...
        let abs_error = (est - true_count as f64).abs();
...
fn run_zipf_stream_regular(
    ...
    samples: usize,
    seed: u64,
) -> (Count<Vector2D<i32>, RegularPath>, HashMap<u64, i32>) {
...
fn run_zipf_stream_fast(
    ...
    samples: usize,
    seed: u64,
) -> (Count<Vector2D<i32>, FastPath>, HashMap<u64, i32>) {
    let mut truth = HashMap::<u64, i32>::new();
...
mod tests {
    ...
    fn path_parity_sanity_writes_both_paths() {
        let unique = SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        let out_root = std::env::temp_dir().join(format!("count_guarantee_parity_{unique}"));
    ...
    fn quick_cli_smoke_creates_only_metrics_csv() {
        let unique = SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        let out_root = std::env::temp_dir().join(format!("count_guarantee_smoke_{unique}"));
        let cli = Cli {
            rows: DEFAULT_ROWS,
            cols: DEFAULT_COLS,
            out_dir: out_root.clone(),
